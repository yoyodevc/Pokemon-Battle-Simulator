...
export default function OnlineBattle({ match: m, user, busy, connected, run, onNotice, onMatch }: Props) {
  ...
  if (m.status === 'battle' || m.status === 'ended') return <OnlineBattleScene match={m} user={user} busy={busy} connected={connected} run={run} onMatch={onMatch} />;
  return <section className="lg-match">
    ...
    {m.status === 'abandoned' ? <div className="lg-empty"><h2>Room closed</h2><p>No battle was recorded.</p><a href="#league" className="lg-primary">Return to lobby</a></div> : ['preparing', 'loading'].includes(m.status) ? <form className="lg-preparation" onSubmit={ready}>
      ...
      <div className="lg-readybar"><span>{m.status === 'loading' ? 'Preparing battle data...' : m.ready[m.side] ? 'Team locked. Waiting for your rival.' : roster.length !== 6 ? `Choose ${6 - roster.length} more Pokémon to complete your team.` : 'Your selections stay private until battle.'}</span><button className="lg-primary" disabled={busy || !connected || roster.length !== 6 || m.ready[m.side] || m.status === 'loading'}>{readyPending ? <span className="lg-loading-label"><span className="tiny-ball lg-ball-spin" aria-hidden="true" /> {m.status === 'loading' ? 'Preparing battle...' : 'Locking in...'}</span> : m.ready[m.side] ? 'Ready' : 'Lock team & ready up'} &#8594;</button><button className="lg-link" type="button" onClick={() => setConfirmForfeit(true)}>Leave room</button></div>
    </form> : <>
