...
export default function TeamSelection({ roster, picker, disabled, error, onChange }: { roster: string[]; picker: PickerPokemon[]; disabled: boolean; error: string; onChange: (roster: string[]) => void }) {
  ...
  return <div className="lg-team-selection">
    <div className="setup-heading"><div><p className="eyebrow">ONLINE / TEAM SELECTION</p><h2>ASSEMBLE YOUR SIX.</h2><p>Your first Pokémon leads. Your lineup stays private until battle.</p></div></div>
    <div className="lobby-layout">
      ...
      <section className="picker-section" aria-labelledby="online-picker-title">
        <div className="picker-heading"><div><p className="eyebrow">THE NATIONAL POKÉDEX</p><h2 id="online-picker-title">PICK YOUR CONTENDERS.</h2></div></div>
        <div className="picker-search"><label className="sr-only" htmlFor="online-picker-search">Search Pokémon</label><input ref={search} id="online-picker-search" onKeyDown={e => { if (e.key === 'Enter') e.preventDefault(); }} placeholder="Search name or Pokédex number…" value={query} onChange={e => { setQuery(e.target.value); setVisible(36); }} /></div>
        ...
        <div className="picker-grid">{filtered.slice(0, visible).map(p => <button className={`picker-card ${roster.includes(p.name) ? 'is-selected' : ''}`} type="button" key={p.name} disabled={disabled || (!roster.includes(p.name) && roster.length === 6)} aria-pressed={roster.includes(p.name)} onClick={() => toggle(p.name)}>
          <span className="picker-number">{String(p.id).padStart(3, '0')}</span><img loading="lazy" src={p.sprite} alt="" /><strong>{p.name.replaceAll('-', ' ')}</strong><span className="picker-action">{roster.includes(p.name) ? 'IN YOUR LINEUP ✓' : roster.length === 6 ? 'TEAM FULL' : '+ ADD TO TEAM'}</span>
        </button>)}</div>
